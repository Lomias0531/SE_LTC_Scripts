namespace BulletXNA.BulletCollision
{
    ...
    public struct AABB
    {
        ...
        {
            this.GetCenterExtend(out IndexedVector3 vector2, out IndexedVector3 vector);
            ...
            float num4 = (vdir.Y * num3) - (vdir.Z * num2);
            if (Math.Abs(num4) > ((vector.Y * Math.Abs(vdir.Z)) + (vector.Z * Math.Abs(vdir.Y))))
            {
                return false;
            }
            num4 = (vdir.Z * x) - (vdir.X * num3);
            if (Math.Abs(num4) > ((vector.X * Math.Abs(vdir.Z)) + (vector.Z * Math.Abs(vdir.X))))
            {
                return false;
            }
            num4 = (vdir.X * num2) - (vdir.Y * x);
            if (Math.Abs(num4) > ((vector.X * Math.Abs(vdir.Y)) + (vector.Y * Math.Abs(vdir.X))))
            {
                return false;
            }
            return true;
