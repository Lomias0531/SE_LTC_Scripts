            ...
            using (MemoryStream stream = new MemoryStream(byteArray))
            {
                using (BinaryReader reader = new BinaryReader((Stream) stream))
                {
                    ...
                    for (int i = 0; i < this.m_num_nodes; i++)
                    {
                        ...
                        {
                            item.m_escapeIndexOrDataIndex[j] = reader.ReadInt32();
                        }
        ...
        internal byte[] Save()
        {
            ...
            using (MemoryStream stream = new MemoryStream())
            {
                using (BinaryWriter writer = new BinaryWriter((Stream) stream))
                {
                    ...
                    for (int i = 0; i < this.m_num_nodes; i++)
                    {
                        writer.Write(this.m_node_array[i].m_escapeIndexOrDataIndex.Length);
                        for (int j = 0; j < this.m_node_array[i].m_escapeIndexOrDataIndex.Length; j++)
                        {
                            writer.Write(this.m_node_array[i].m_escapeIndexOrDataIndex[j]);
                        }
                        WriteUShortVector3(this.m_node_array[i].m_quantizedAabbMin, writer);
                        WriteUShortVector3(this.m_node_array[i].m_quantizedAabbMax, writer);
                    }
