...
namespace LitJson
{
	public class JsonMapper
	{
		...
		private static readonly IDictionary<string, ArrayMetadata> array_metadata;
		...
		private static readonly IDictionary<string, ObjectMetadata> object_metadata;
		...
		static JsonMapper()
		{
			...
			array_metadata = new Dictionary<string, ArrayMetadata>();
			conv_ops = new Dictionary<Type, IDictionary<Type, MethodInfo>>();
			object_metadata = new Dictionary<string, ObjectMetadata>();
		...
		{
			if (array_metadata.ContainsKey(type.FullName))
			{
				return;
			}
			...
			lock (array_metadata_lock)
			{
				try
				{
					array_metadata.Add(type.FullName, value);
				}
		...
		{
			if (object_metadata.ContainsKey(type.FullName))
			{
				return;
			}
			...
			lock (object_metadata_lock)
			{
				try
				{
					object_metadata.Add(type.FullName, value);
				}
				catch (ArgumentException)
				{
				}
		...
		private static object ReadValue(Type inst_type, JsonReader reader)
		{
			...
			if (reader.Token == JsonToken.ArrayStart)
			{
				...
				ArrayMetadata arrayMetadata = array_metadata[inst_type.FullName];
			...
			else if (reader.Token == JsonToken.ObjectStart)
			{
				...
				ObjectMetadata objectMetadata = object_metadata[type.FullName];
