		...
		public override SyntaxNode VisitAttribute(AttributeSyntax node)
		{
			...
			if (namedTypeSymbol == m_protoMemberAttributeType)
			{
				...
				if (!separatedSyntaxList.HasValue || separatedSyntaxList.Value.Count == 0 || !HasTagArgument(separatedSyntaxList.Value))
				{
					...
					AttributeArgumentSyntax[] first = new AttributeArgumentSyntax[1]
					{
						SyntaxFactory.AttributeArgument(SyntaxFactory.LiteralExpression(SyntaxKind.NumericLiteralExpression, SyntaxFactory.Literal(node.GetLocation().GetLineSpan().StartLinePosition.Line)))
					};
