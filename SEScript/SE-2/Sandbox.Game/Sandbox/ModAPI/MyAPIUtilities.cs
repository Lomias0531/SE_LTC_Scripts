...
namespace Sandbox.ModAPI
{
	public class MyAPIUtilities : IMyUtilities, IMyGamePaths
	{
		...
		{
			value = default(T);
			if (Variables.TryGetValue(name, out object value2) && value2 is T)
			{
				value = (T)value2;
				return true;
			}
		...
		{
			if (m_registeredListeners.TryGetValue(id, out List<Action<object>> value))
			{
				value.Add(messageHandler);
			}
			else
			{
				m_registeredListeners[id] = new List<Action<object>>
				{
					messageHandler
				};
			}
