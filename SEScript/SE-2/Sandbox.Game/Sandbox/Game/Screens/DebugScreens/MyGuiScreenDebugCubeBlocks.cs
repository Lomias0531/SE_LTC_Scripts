	...
	internal class MyGuiScreenDebugCubeBlocks : MyGuiScreenDebugBase
	{
		...
		private void onClick_Save(MyGuiControlButton sender)
		{
			foreach (MyDefinitionBase allDefinition in MyDefinitionManager.Static.GetAllDefinitions())
			{
				...
				{
					MyCubeBlockDefinition.MountPoint[] source = MyCubeBuilder.AutogenerateMountpoints(MyModels.GetModel(myCubeBlockDefinition.Model), MyDefinitionManager.Static.GetCubeSize(myCubeBlockDefinition.CubeSize));
					myCubeBlockDefinition.MountPoints = source.ToArray();
				}
			}
